from chat.models import ConversationRequest, Message, Room
...
def create_group(teacher_id, course_id) -> GroupAnnotation:
    group = Group.objects.create(group_name="Teacher", teacher=teacher_id, course=course_id)
    return group
...
def create_article(teacher_id, topic_id) -> ArticleAnnotation:
    article = Article.objects.create(title="Test", teacher=teacher_id, topic=topic_id, content="123456")
    return article


def create_test(teacher_id, topic_id) -> TestAnnotation:
    test = Test.objects.create(title="Test", teacher=teacher_id, topic=topic_id, description="123456", is_open=False)
    return test


def create_question(test_id) -> QuestionAnnotation:
    question = Question.objects.create(text="test_text", test=test_id, is_important=True)
    return question


def create_answer(question_id) -> AnswerAnnotation:
    answer = Answer.objects.create(text="answer", question=question_id, is_correct=True)
    return answer
...
def create_email(user_id) -> EmailAnnotation:
    email = Email.objects.create(sender=user_id, recipients=user_id, subject="issue", message="1+2=?")
    return email


def create_room(user):
    room = Room.objects.create(name="Lucky")
    room.online.add(user)
    return room


def create_message(user, room):
    message = Message.objects.create(room=room, user=user, content="Very interesting")
    return message


def create_conversationrequest(user_id):
    user = User.objects.get(pk=user_id)
    conversationrequest = ConversationRequest.objects.create(from_user=user, to_user=user, message="Test for my chat")  # Например, отправляем запрос самому себе
